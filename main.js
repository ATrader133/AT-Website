document.addEventListener('DOMContentLoaded', function() {
    console.log("ABRAR TRADERS: System Loaded v9.0 (Clean Architecture)");
    ...
    window.drawDieCut = () => {
        ...
        if (viewer) {
            viewer.scale = `${l / 1000} ${h / 1000} ${w / 1000}`;
        }
        ...
        basket.forEach((item, index) => {
            list.innerHTML += `
                <li class="flex justify-between items-center bg-gray-50 dark:bg-slate-800 p-3 rounded-lg border border-gray-100 dark:border-slate-700">
                    ...
                    <button onclick="removeFromBasket(${index})" class="text-red-400 hover:text-red-600 p-1 font-bold">✖</button>
                </li>`;
        });
    };
    ...
    window.switchTab = function(tabName) {
        ...
        if(activeBtn) {
            ...
            if(tabName === 'diecut') { colorClass = "text-purple-600"; setTimeout(window.drawDieCut, 50); }
    ...
    window.switchTab('weight'); // Init Tab
    ...
    function showQuizResult() {
        ...
        const step3 = document.getElementById('quiz-step-3');
        if(step3) { step3.classList.remove('hidden'); void step3.offsetWidth; step3.style.opacity = '1'; }
    }
    ...
    if (typeof VanillaTilt !== 'undefined') {
        VanillaTilt.init(document.querySelectorAll(".product-card"), { max: 12, speed: 400, glare: true, "max-glare": 0.2, perspective: 1000 });
    }
    ...
    if (window.tsParticles && document.getElementById('tsparticles') && !window.particlesLoaded) {
        ...
        tsParticles.load("tsparticles", {
            particles: {
                ...
                color: { value: ["#3B82F6", "#8B5CF6", "#10B981", "#ffffff", "#cbd5e1"] },
                shape: { type: "polygon", polygon: { nb_sides: 4 } },
                opacity: { value: 0.6, random: true }, size: { value: 8, random: true },
                move: { enable: true, speed: 1.5, direction: "bottom", random: true, straight: false, out_mode: "out", bounce: false },
                rotate: { value: 0, random: true, direction: "random", animation: { enable: true, speed: 10 } }
            },
            interactivity: {
                detect_on: "canvas",
    ...
    // ==========================================
    // 10. UI COMPONENTS & SCROLLING
    // ==========================================
    const sidebar = document.getElementById('socialSidebar');
    if (sidebar) { setTimeout(() => sidebar.classList.add('peek-out'), 1000); setTimeout(() => sidebar.classList.remove('peek-out'), 3500); }
    ...
    if (quoteForm) {
        quoteForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    window.showToast("Quote requested successfully!", "success"); quoteForm.reset();
                    if (typeof basket !== 'undefined') { basket = []; localStorage.setItem('at_basket', JSON.stringify(basket)); document.getElementById('basketCount').textContent = '0'; document.getElementById('quoteBasket').classList.add('hidden'); }
    ...
    if (scene && cube) {
        ...
        scene.addEventListener('touchmove', (e) => {
            ...
            const touch = e.touches[0]; const rect = scene.getBoundingClientRect();
            const currentX = touch.clientX - rect.left; const currentY = touch.clientY - rect.top;
    ...
    if (window.matchMedia("(min-width: 768px)").matches) {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            if (speed > 8) { 
                particles.push({ x: mouseX - Math.cos(currentAngle * (Math.PI / 180)) * 20, y: mouseY - Math.sin(currentAngle * (Math.PI / 180)) * 20, radius: Math.min(speed * 0.4, 15) + 5, opacity: 0.6, growth: 0.3, fadeRate: 0.015 });
            }
        });
    ...
    // ==========================================
    // 16. ESG CALCULATOR & NUMBER ANIMATION
    // ==========================================
    ...
    function animateValue(id, start, end, duration) {
        const obj = document.getElementById(id); if(!obj || start === end) return;
        let startTimestamp = null;
        const step = (timestamp) => {
            if (!startTimestamp) startTimestamp = timestamp;
            const progress = Math.min((timestamp - startTimestamp) / duration, 1);
            obj.innerHTML = Math.floor((1 - Math.pow(1 - progress, 3)) * (end - start) + start).toLocaleString(); 
    ...
    // ==========================================
    // 17. CONTAINER LOAD SIMULATOR LOGIC
    // ==========================================
        ...
        link.addEventListener('click', e => {
            ...
            if (target && target.includes('.html') && !target.startsWith('http')) {
                if (!document.startViewTransition) {
                    ...
                    document.body.style.opacity = '0'; document.body.style.transform = 'scale(0.98)';
    ...
    // ==========================================
    // 19. ESG PDF GENERATOR
    // ==========================================
    document.getElementById('esgTons')?.addEventListener('input', (e) => {
        const tons = parseFloat(e.target.value) || 0; const btn = document.getElementById('esgDownloadBtn');
    ...
    // ==========================================
    // 21. TRUE AI SUPPLY CHAIN ASSISTANT
    // ==========================================
    ...
    window.handleChatEnter = (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); window.sendChatMessage(); } };
    ...
    window.sendChatMessage = async () => {
        ...
        try {
            ...
            const actionHtml = `<div class="mt-4 flex gap-2 border-t border-blue-200/50 dark:border-slate-700 pt-3"><a href="#quote" onclick="toggleChat()" class="text-xs bg-white dark:bg-slate-700 text-blue-600 dark:text-white px-3 py-1.5 rounded-md font-bold shadow-sm">Get a Quote</a><a href="#calculator" onclick="toggleChat(); window.switchTab('weight');" class="text-xs bg-white dark:bg-slate-700 text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-md font-bold shadow-sm">Calculate Weight</a></div>`;
    ...
    document.addEventListener('keydown', (e) => {
        if ((e.ctrlKey || e.metaKey) && e.key === '/') { e.preventDefault(); window.toggleChat(); }
    });

}); // <-- CRITICAL: THIS IS THE ONLY MAIN SCRIPT CLOSURE.
...
window.addEventListener('load', () => {
    ...
    const imageObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) { entry.target.classList.add('loaded'); observer.unobserve(entry.target); }
        });
