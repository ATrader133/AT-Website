// ======================================================================
// ABRAR TRADERS: BULLETPROOF ARCHITECTURE v10
// Eliminates DOMContentLoaded Race Conditions & Global Scope Errors
// ======================================================================
console.log("ABRAR TRADERS: Core Script Initializing...");
...
// ==========================================
// 2. GLOBAL FUNCTIONS (Instantly Bound to Window)
// ==========================================
...
window.closeModal = function() {
    const pm = document.getElementById('productModal'); const pmb = document.getElementById('productModalBackdrop');
    if(pm) { pm.classList.remove('hidden'); setTimeout(() => pm.classList.add('hidden'), 300); }
...
window.calculateCBM = function() {
    ...
    if (fillBar) {
        setTimeout(() => {
            ...
            fillBar.className = "absolute top-0 left-0 h-full transition-all duration-1000 ease-out flex items-center justify-end overflow-hidden shadow-[inset_-5px_0_15px_rgba(0,0,0,0.3)] " + (fillPercentage === 100 ? "bg-red-500" : fillPercentage > 85 ? "bg-green-500" : "bg-blue-500");
            if(percentText) percentText.style.color = fillPercentage === 100 ? "#ef4444" : fillPercentage > 85 ? "#10b981" : "#3b82f6";
        }, 50);
    }
    if (percentText) percentText.innerText = fillPercentage.toFixed(1) + '%';
...
// ==========================================
function initAbrarSystem() {
    console.log("ABRAR TRADERS: DOM Ready. Booting Engine...");
...
// ==========================================
// 4. THE LAUNCH MECHANISM (Defeats Race Conditions)
// ==========================================
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initAbrarSystem);
} else {
    // If the browser already finished parsing HTML before the script loaded (because of defer), launch instantly.
    initAbrarSystem();
}
